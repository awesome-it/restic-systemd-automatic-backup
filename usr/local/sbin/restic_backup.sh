...
if [[ -n "$BACKUP_PROMETHEUS_TXT_COLLECTOR" ]] ; then
  ...
  $restic_bin stats --json latest | jq -r '"restic_stats_last_snapshot_total_size_bytes \(.total_size)\nrestic_stats_last_snapshot_file_count \(.total_file_count)"' > $BACKUP_PROMETHEUS_TXT_COLLECTOR/${PREFIX}restic-stats-latest-snapshot.prom.$$
  ...
  $restic_bin stats --mode raw-data --json | jq -r '"restic_stats_total_size_bytes \(.total_size)"' > $BACKUP_PROMETHEUS_TXT_COLLECTOR/${PREFIX}restic-stats-total.prom.$$
  ...
  $restic_bin snapshots --json latest | jq -r '.[].time | split(".")[0] | strptime("%Y-%m-%dT%H:%M:%S") | mktime | "restic_stats_last_snapshot_timestamp \(.)"' > $BACKUP_PROMETHEUS_TXT_COLLECTOR/${PREFIX}restic-stats-latest-snapshot-timestamp.prom.$$
